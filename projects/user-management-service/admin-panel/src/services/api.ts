...
export const userApi = {
  ...
  resetPassword: (id: string, newPassword: string): Promise<ApiResponse> =>
    apiCall('POST', `/users/${id}/reset-password`, { password: newPassword }),
...
export const teamApi = {
  ...
  getTeamMembers: (teamId: string): Promise<ApiResponse<User[]>> =>
    apiCall('GET', `/teams/${teamId}/members`),
  
  addTeamMember: (teamId: string, userId: string, role: string): Promise<ApiResponse> =>
    apiCall('POST', `/teams/${teamId}/members`, { userId, role }),
  
  removeTeamMember: (teamId: string, userId: string): Promise<ApiResponse> =>
    apiCall('DELETE', `/teams/${teamId}/members/${userId}`),
  
  updateMemberRole: (teamId: string, userId: string, role: string): Promise<ApiResponse> =>
    apiCall('PUT', `/teams/${teamId}/members/${userId}`, { role }),
}
